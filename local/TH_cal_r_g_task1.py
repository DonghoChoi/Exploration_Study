...
if __name__ == "__main__":
    ...
    for i in range(0,len(TH_task1_complete_user_list)): # i - current userID
        ...
        sql = "SELECT label,COUNT(label) as label_count,SUM(duration) as sum_duration FROM user_TH_merged_results WHERE(userID={0} AND behavior='InfoPatch' AND duration>{1} AND observed_time<{2}) GROUP BY label ORDER BY COUNT(label) DESC, SUM(duration) DESC".format(current_userID,infoPatch_threshold,T1_done_time)
        ...
        sql = "INSERT INTO TH_mobility_data_task1 (userID,visited_locations,k,visited_locations_k,gyration_all,gyration_k,s_k) VALUES ({0},{1},{2},{3},{4},{5},{6})".\
